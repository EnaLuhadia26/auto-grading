...
# ✅ Organizing QA Bank into categories
qa_bank = {
    "Python": {
...
def home():
    # Show subject list
    return render_template('index.html', subjects=list(qa_bank.keys()))

@app.route('/get_question/<subject>/<int:q_index>', methods=['GET'])
def get_question(subject, q_index):
    if subject not in qa_bank:
        return jsonify({"error": "Subject not found!"})
    
    questions = list(qa_bank[subject].keys())
    if q_index < len(questions):
        ...
        return jsonify({"question": None, "message": "Quiz finished!"})
...
def grade():
    ...
    if not subject or not question or not answer:
        return jsonify({"score": 0, "feedback": "Please provide subject, question and answer."})

    if subject not in qa_bank or question not in qa_bank[subject]:
        return jsonify({"score": 0, "feedback": "Question not found in database."})

    correct_answer = qa_bank[subject][question]
